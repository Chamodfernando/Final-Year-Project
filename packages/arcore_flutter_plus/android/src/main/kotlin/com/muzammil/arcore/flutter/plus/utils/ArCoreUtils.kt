package com.muzammil.arcore.flutter.plus.utils
    ...
    companion object {
        ...
        fun createArSession(activity: Activity, userRequestedInstall: Boolean, isFrontCamera: Boolean): Session? {
            ...
            if (hasCameraPermission(activity)) {
                ...
                session?.let { s ->
                    fun firstConfigOrNull(filter: CameraConfigFilter): CameraConfig? {
                        val list = s.getSupportedCameraConfigs(filter)
                        return list.firstOrNull()
                    }

                    // Prefer 30fps + no depth (battery friendly), then relax until we get a config.
                    val strict = CameraConfigFilter(s).apply {
                        setTargetFps(EnumSet.of(CameraConfig.TargetFps.TARGET_FPS_30))
                        setDepthSensorUsage(EnumSet.of(CameraConfig.DepthSensorUsage.DO_NOT_USE))
                    }
                    var chosen = firstConfigOrNull(strict)
                    if (chosen == null) {
                        val fpsLoose = CameraConfigFilter(s).apply {
                            setTargetFps(EnumSet.of(
                                    CameraConfig.TargetFps.TARGET_FPS_30,
                                    CameraConfig.TargetFps.TARGET_FPS_60))
                            setDepthSensorUsage(EnumSet.of(CameraConfig.DepthSensorUsage.DO_NOT_USE))
                        }
                        chosen = firstConfigOrNull(fpsLoose)
                    }
                    if (chosen == null) {
                        val depthOk = CameraConfigFilter(s).apply {
                            setTargetFps(EnumSet.of(CameraConfig.TargetFps.TARGET_FPS_30))
                            setDepthSensorUsage(EnumSet.of(
                                    CameraConfig.DepthSensorUsage.DO_NOT_USE,
                                    CameraConfig.DepthSensorUsage.REQUIRE_AND_USE))
                        }
                        chosen = firstConfigOrNull(depthOk)
                    }
                    if (chosen == null) {
                        chosen = firstConfigOrNull(CameraConfigFilter(s))
                    }
                    if (chosen != null) {
                        s.cameraConfig = chosen
                    } else {
                        Log.w(TAG, "No supported camera configs; session may not track (no dots/planes)")
                    }
                }
