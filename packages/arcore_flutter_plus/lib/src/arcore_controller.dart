...
//  UnsupportedHandler onUnsupported;
  ...
  /// Warms Sceneform's model pipeline while the user scans surfaces so the first tap shows faster.
  Future<void> prefetchReferenceModel({
    String? object3DFileName,
    String? objectUrl,
  }) {
    return _channel.invokeMethod<void>('prefetchReferenceModel', {
      'object3DFileName': object3DFileName,
      'objectUrl': objectUrl,
    });
  }

  /// Hides plane mesh and stops plane events to Flutter — improves framerate while inspecting a placed model.
  Future<void> setLightweightSceneMode(bool enabled) {
    return _channel.invokeMethod<void>('setLightweightSceneMode', {
      'enabled': enabled,
    });
  }

  /// Updates [scale] and/or [rotation] on an existing scene node (e.g. anchored model).
  Future<void> updateNodeTransform({
    required String nodeName,
    Vector3? scale,
    Vector4? rotation,
  }) {
    final Map<String, dynamic> args = <String, dynamic>{'nodeName': nodeName};
    final sm = scale != null ? convertVector3ToMap(scale) : null;
    final rm = rotation != null ? convertVector4ToMap(rotation) : null;
    if (sm != null) args['scale'] = sm;
    if (rm != null) args['rotation'] = rm;
    return _channel.invokeMethod<void>('updateNodeTransform', args);
  }
